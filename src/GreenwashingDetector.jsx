  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f0fdf4 0%, #f0f9ff 50%, #fdf4ff 100%)", fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #064e3b 0%, #065f46 40%, #0f766e 100%)", padding: "48px 24px 80px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", top: 20, left: "40%", width: 150, height: 150, borderRadius: "50%", background: "rgba(255,255,255,0.02)" }} />
        <div style={{ maxWidth: 900, margin: "0 auto", position: "relative" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.65)", fontSize: 16, margin: "0 0 32px", maxWidth: 500 }}>
            Upload any ESG report and instantly detect greenwashing patterns using our AI-powered keyword analysis engine.
          </p>
      ...
      <div style={{ maxWidth: 900, margin: "-28px auto 0", padding: "0 24px", position: "relative", zIndex: 10 }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: 6, display: "inline-flex", gap: 4, boxShadow: "0 4px 24px rgba(0,0,0,0.08), 0 1px 4px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>
            ...
            <div>
              <div
                ...
                style={{
                  background: dragOver ? "linear-gradient(135deg, #ecfdf5, #f0fdf4)" : "#fff",
              ...
              >
                {loading ? (
                  <div>
                    ...
                    <p style={{ color: "#059669", fontWeight: 600, fontSize: 16 }}>Extracting text from PDF...</p>
                    <p style={{ color: "#9ca3af", fontSize: 13 }}>This may take a moment for large files</p>
                  </div>
                ) : fileName && text ? (
                  <div>
                    ...
                    <button onClick={reset} style={{ background: "none", border: "1px solid #d1d5db", borderRadius: 8, padding: "6px 16px", color: "#6b7280", cursor: "pointer", fontSize: 13 }}>
                      Remove & Upload New
                    </button>
                  </div>
                ) : (
                  <div>
                    ...
                    <label style={{
                      display: "inline-block", background: "linear-gradient(135deg, #064e3b, #0f766e)",
              ...
              {text && !loading && (
                <button onClick={analyze} style={{
                  marginTop: 16, width: "100%", background: "linear-gradient(135deg, #064e3b, #0f766e)",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
                ...
                {[
                  ...
                  <div key={n} style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                    ...
                    <div>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#111827" }}>{t}</p>
                      <p style={{ margin: 0, fontSize: 12, color: "#9ca3af" }}>{d}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: "linear-gradient(135deg, #ecfdf5, #f0fdf4)", border: "1px solid #a7f3d0", borderRadius: 16, padding: 20 }}>
                ...
                <p style={{ margin: 0, fontSize: 12, color: "#047857", lineHeight: 1.6 }}>
                  Studies show that over 40% of green claims made online are exaggerated, false, or deceptive. Our tool helps you cut through the noise.
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === "keywords" && (
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: 24, marginBottom: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <select
                  ...
                  style={{ flex: 1, padding: "10px 12px", border: "1px solid #e5e7eb", borderRadius: 10, fontFamily: "inherit", fontSize: 14, color: "#374151", background: "#f9fafb" }}
                >
                  <option value="">Select category...</option>
                  {Object.keys(keywords).map(cat => <option key={cat} value={cat}>{cat}</option>)}
                  <option value="__new__">+ New category</option>
                </select>
                {newKeywordCategory === "__new__" && (
                  <input
                ...
                <button onClick={addKeyword} style={{
                  ...
                  fontFamily: "inherit", fontWeight: 700, cursor: "pointer", fontSize: 14, whiteSpace: "nowrap"
                }}>+ Add</button>
              </div>
            </div>
                ...
                return (
                  <div key={cat} style={{ background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.04)", border: "1px solid #f3f4f6" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                      ...
                      <span style={{ background: colors.bg, color: colors.text, border: `1px solid ${colors.border}`, borderRadius: 20, padding: "2px 10px", fontSize: 11, fontWeight: 600 }}>
                        {kws.length}
                      </span>
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {kws.map(kw => (
                        <div key={kw} style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 8, padding: "4px 10px", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <button onClick={() => removeKeyword(cat, kw)} style={{ background: "none", border: "none", color: colors.text, cursor: "pointer", padding: 0, fontSize: 13, opacity: 0.6, lineHeight: 1 }}>✕</button>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div style={{ marginTop: 16, textAlign: "right" }}>
              <button onClick={() => setKeywords(JSON.parse(JSON.stringify(DEFAULT_GREENWASHING_KEYWORDS)))} style={{
                background: "none", border: "1px solid #e5e7eb", borderRadius: 10,
                padding: "8px 16px", color: "#6b7280", cursor: "pointer", fontFamily: "inherit", fontSize: 13
          ...
          <div>
            {!report ? (
              ...
              <>
                {/* Score Hero */}
                <div style={{
                  background: `linear-gradient(135deg, ${report.risk.bg}, #fff)`,
                  border: `1px solid ${report.risk.border}`,
                  ...
                  <div style={{ marginTop: 24 }}>
                    <div style={{ height: 8, background: "#e5e7eb", borderRadius: 4, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${report.riskScore}%`, background: `linear-gradient(90deg, #22c55e, ${report.risk.color})`, borderRadius: 4, transition: "width 1s" }} />
                    </div>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginTop: 20 }}>
                    {[
                      { v: report.total, l: "Total occurrences" },
